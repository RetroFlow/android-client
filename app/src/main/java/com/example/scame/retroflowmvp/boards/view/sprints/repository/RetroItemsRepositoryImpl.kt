...
class RetroItemsRepositoryImpl: RetroItemsRepository {
    ...
    override fun getRetroSectionsForBoard(boardId: String): Single<List<RetroSection>> {
        val issue1 = ActionItem(1.toString(), "Check backups regularly", "Should be done on daily-basis", listOf(), null, false)
        val issue2 = ActionItem(2.toString(), "Stop polluting announcement channel", "Please, use only for real announcements", listOf(), "All", false)
        val issue3 = ActionItem(3.toString(), "Clarify scope meetings", "Make sure that everyone know what's going on here", listOf(), "RM", false)

        val issue4 = ActionItem(4.toString(), "Start writing rough estimates on tickets", "makes overall picture clearer", listOf(), "developers", true)
        val issue5 = ActionItem(5.toString(), "Start using online tools for retrospectives", "process would be simpler and more effective", listOf(), "RM", true)
        val issue6 = ActionItem(6.toString(), "Continue running nightly error checkers", "finally it's automated", listOf(), "Ops", false)
        val issue7 = ActionItem(7.toString(), "Stop inviting all people that could be possibly interested (any meeting)", "Ineffective and annoying", listOf(), "PM & RM", false)
